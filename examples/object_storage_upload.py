...
Run from the repository root (mnemospark/):
  ...
  python examples/object_storage_upload.py upload <wallet-address> <object-id> [--location REGION]
  python examples/object_storage_upload.py ls <wallet-address> <object-id> [--location REGION]
  python examples/object_storage_upload.py download <wallet-address> <object-id> [--location REGION]
  python examples/object_storage_upload.py delete <wallet-address> <object-id> [--location REGION]

For upload, object-id is a local file path. For ls/download/delete, object-id is the S3 object key.
"""
...
def list_object(
    ...
    """List object in S3 (name + size). Returns result dict with success, key, size_bytes, error."""
    ...
    try:
        ...
        resp = s3_client.head_object(Bucket=bucket_name_str, Key=object_id)
        size = resp.get("ContentLength", 0)
        return {"success": True, "key": object_id, "size_bytes": size, "bucket": bucket_name_str}
    except ValueError as e:
        return {"success": False, "error": str(e)}
    except ClientError as e:
        if e.response["Error"].get("Code") == "404":
            return {"success": False, "error": f"Object not found: {object_id}.", "not_found": True}
        return {"success": False, "error": f"S3 error: {e.response['Error'].get('Message', str(e))}"}
...
def download_object(
    ...
    try:
        ...
        kek = load_or_create_kek(wh)
        resp = s3_client.get_object(Bucket=bucket_name_str, Key=object_id)
        body = resp["Body"].read()
...
def delete_object(
    ...
    """Delete object from S3. Optionally check existence first. Returns result dict."""
    if not object_id or "/" in object_id or object_id in (".", ".."):
        return {"success": False, "error": "Invalid object key."}
    if s3_client is None:
        s3_client = boto3.client("s3", region_name=location)
    ...
    try:
        ...
        require_bucket_exists(s3_client, bucket_name_str)
        try:
            s3_client.head_object(Bucket=bucket_name_str, Key=object_id)
        except ClientError as e:
            if e.response["Error"].get("Code") == "404":
                return {"success": False, "error": f"Object not found: {object_id}.", "not_found": True}
            raise
        s3_client.delete_object(Bucket=bucket_name_str, Key=object_id)
        return {"success": True, "key": object_id, "bucket": bucket_name_str}
    except ValueError as e:
        return {"success": False, "error": str(e)}
    except ClientError as e:
...
def send_log_event(
    ...
    try:
        ...
        # Fallback: log to stderr so user still has a record
...
def main() -> int:
    ...
    for cmd in ("upload", "ls", "download", "delete"):
        ...
        sub.add_argument("wallet_address", help="Base blockchain wallet address")
        sub.add_argument("object_id", help="upload: local file path; ls/download/delete: S3 object key")
        sub.add_argument(
            "--location",
            default="us-east-1",
    ...
    location = getattr(args, "location", "us-east-1") or "us-east-1"
    ...
    if command == "upload":
        ...
        result = list_object(wallet_address, object_id, location)
    elif command == "download":
        result = download_object(wallet_address, object_id, location)
    elif command == "delete":
        result = delete_object(wallet_address, object_id, location)
    else:
        print(f"Error: Unknown command {command}", file=sys.stderr)
        return EXIT_VALIDATION

    send_log_event(command, wallet_address, object_id, location, result)
    ...
    if result.get("success"):
        if command == "upload":
            ...
            print(f"{result['key']}  {size_str}")
        elif command == "download":
            print(f"Downloaded {result['key']} to {result['path']}.")
        elif command == "delete":
            print(f"Object deleted from S3: {result['key']}.")
        return EXIT_SUCCESS
