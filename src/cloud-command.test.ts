...
describe("cloud command", () => {
  ...
  it("handles /mnemospark cloud delete, removes cron job, and prints two user messages", async () => {
    ...
    const cronAdapter = createInMemoryCronAdapter([
      {
        jobId: cronId,
        message: `Execute: node ${join(homeDir, ".openclaw/extensions/mnemospark/dist/cli.js")} cloud payment-settle --renewal --wallet-address "0x1234abcd" --object-id "obj-001" --object-key "backup/archive.tar.gz" --storage-price "2.75"`,
      },
    ]);
        ...
        list: async () => [
          {
            ...
            message: `Execute: node ${join(homeDir, ".openclaw/extensions/mnemospark/dist/cli.js")} cloud payment-settle --renewal --wallet-address "0x1234abcd" --object-id "obj-throws-001" --object-key "backup/other.tar.gz" --storage-price "2.75"`,
          },
        ],
      }),
      requestStorageDeleteFn: async () => ({
